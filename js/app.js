...
/**
 * Scrolls the in-page player profile card into view (players section).
 * @param {{ forModalOpen?: boolean }} [options] — if forModalOpen, uses instant scroll so layout + modal open stay reliable (smooth scroll can be interrupted).
 */
function scrollPesPlayerProfileIntoViewIfVisible(options = {}) {
	const forModalOpen = options.forModalOpen === true;
	const section = document.querySelector('[data-pes-section="players"]');
	const el = document.getElementById("pes-player-profile-root");
	if (!section || section.classList.contains("hidden") || !el) {
		return;
	}
	// Ensure geometry reflects latest innerHTML (e.g. right after renderPlayersView).
	void el.offsetHeight;
	const rect = el.getBoundingClientRect();
	if (rect.height < 2 && !el.textContent.trim()) {
		return;
	}
	let reduce = false;
	try {
		const s =
			typeof window.getPesAppSettings === "function"
				? window.getPesAppSettings()
				: null;
	...
	const useInstant = forModalOpen || reduce;
	const behavior = useInstant ? "auto" : "smooth";
	try {
		el.scrollIntoView({ block: "start", inline: "nearest", behavior });
	} catch {
		try {
			el.scrollIntoView(!useInstant);
		} catch {
			// ignore
		}
	}
	// Reserve: sticky header / scroll-margin quirks — align top edge below header.
	const header = document.querySelector(".pes6-top-bar");
	const headerH = header ? header.getBoundingClientRect().height : 64;
	const gap = 12;
	const sr = document.scrollingElement || document.documentElement;
	let r = el.getBoundingClientRect();
	if (r.top < headerH + gap) {
		const docTop = window.scrollY ?? sr.scrollTop ?? 0;
		sr.scrollTo({
			top: Math.max(0, docTop + r.top - headerH - gap),
			behavior: "auto",
		});
	}
	// Re-measure after nudging; if still clipped at bottom, scroll the viewport once more.
	r = el.getBoundingClientRect();
	const viewH = window.innerHeight || 0;
	if (viewH > 0 && r.bottom > viewH - gap) {
		const docTop = window.scrollY ?? sr.scrollTop ?? 0;
		sr.scrollTo({
			top: Math.max(0, docTop + r.bottom - viewH + gap),
			behavior: "auto",
		});
	}
}
...
function openTeamModal(teamId) {
	...
	document.getElementById("pes-edit-team-founded").value =
		team.foundedYear || "";
...
function openAppSettingsModal() {
	...
	const backupFileInput = document.getElementById("pes-app-backup-file-input");
	if (backupFileInput) {
...
function bindAppSettingsModal() {
	...
	form.addEventListener("submit", (event) => {
		...
		window.updatePesAppSettings({
			...
			confirmDangerousActions: document.getElementById("pes-app-set-confirm")
				.checked,
			toastDurationKey: document.getElementById("pes-app-set-toast-dur").value,
			suppressSuccessToasts: document.getElementById("pes-app-set-hide-success")
				.checked,
			dateFormat: document.getElementById("pes-app-set-date").value,
			exportLeagueSlug: String(
				document.getElementById("pes-app-set-slug").value || "",
			).trim(),
...
function openCloudModal() {
	...
	const introPresetMissing = document.getElementById(
		"pes-cloud-intro-preset-missing",
	);
...
function buildPesLeagueBackupJsonFilename() {
	let slug = "";
	try {
		slug = String(window.getPesAppSettings().exportLeagueSlug || "")
			.trim()
			.replace(/[^\w-]+/g, "_")
			.replace(/^_+|_+$/g, "");
	} catch {
	...
	if (slug) {
		return `pes-liga-${slug}-backup-${dateStr}.json`;
	}
	return `pes-liga-backup-${dateStr}.json`;
}

function bindPesLeagueBackupUi() {
	const downloadBtn = document.getElementById("pes-app-backup-download");
	const fileInput = document.getElementById("pes-app-backup-file-input");
	if (!downloadBtn || !fileInput) {
		return;
	}
	downloadBtn.addEventListener("click", () => {
		const json = exportPesLeagueStateAsJsonString();
		downloadTextFile(
			buildPesLeagueBackupJsonFilename(),
			json,
			"application/json;charset=utf-8",
		);
		showToastMessage(t("toast.backupExported"), "success");
	});
	fileInput.addEventListener("change", () => {
		...
		reader.onload = () => {
			try {
				...
				if (!window.confirmPesDangerous(t("confirm.importBackup"))) {
					fileInput.value = "";
					return;
				}
				importPesLeagueStateFromJsonString(text);
				refreshEntireUi();
				refreshCloudSyncStatusUi();
				showToastMessage(t("toast.backupImported"), "success");
			} catch (err) {
				const msg = err instanceof Error ? err.message : String(err);
				showToastMessage(
					t("error.backupImportFailed", { detail: msg }),
					"error",
				);
			} finally {
				fileInput.value = "";
		...
		reader.onerror = () => {
			showToastMessage(t("error.backupReadFile"), "error");
			fileInput.value = "";
		};
...
function bindGlobalPlayerSearchShortcut() {
	window.addEventListener("keydown", (event) => {
		...
		closeHeaderMobileMenu();
		const viewId =
			typeof getViewIdFromHash === "function"
				? getViewIdFromHash()
				: "dashboard";
		if (viewId !== "players") {
...
function bindForms() {
	...
	if (addPlayerForm) {
		addPlayerForm.addEventListener("submit", async (event) => {
			...
			const state = getPesLeagueApplicationState();
			const chosenExistingId = String(
				formData.get("existingTeamId") || "",
			).trim();
	...
	if (editPlayerForm) {
		editPlayerForm.addEventListener("submit", async (event) => {
			...
			let avatarDataUrl = existingPlayer
				? existingPlayer.avatarDataUrl || ""
				: "";
			try {
...
function mapApiTeamToLocalModel(apiTeam) {
	...
	const constructedByIdCandidates = [
		apiTeam.idTeam
			? `https://www.thesportsdb.com/images/media/team/badge/${apiTeam.idTeam}.png`
			: "",
		apiTeam.idTeam
			? `https://www.thesportsdb.com/images/media/team/logo/${apiTeam.idTeam}.png`
			: "",
		apiTeam.idTeam
			? `https://www.thesportsdb.com/images/media/team/badge/${apiTeam.idTeam}.jpg`
			: "",
	];
...
async function fetchWikipediaTeamLogoUrl(teamName) {
	...
	if (
		Object.prototype.hasOwnProperty.call(PES_WIKIPEDIA_LOGO_CACHE, cacheKey)
	) {
	...
	const baseName = String(teamName || "")
		.trim()
		.replace(/\s+/g, " ");
	const titleCandidates = [`${baseName} F.C.`, `${baseName} FC`, baseName]
		...
		searchForm.addEventListener("submit", async (event) => {
			...
			try {
				...
				setTeamDiscoveryStatus(
					t("discovery.foundCount", { n: String(enrichedTeams.length) }),
					false,
				);
			} catch (error) {
				...
				const msg =
					error instanceof Error ? error.message : t("discovery.unknownErr");
				setTeamDiscoveryStatus(
					t("discovery.searchFailed", { hint, msg }),
					true,
				);
			}
		});
	...
	if (browseForm) {
		browseForm.addEventListener("submit", async (event) => {
			...
			try {
				...
				renderDiscoveredTeamResults(enrichedTeams);
				setTeamDiscoveryStatus(
					t("discovery.leagueCount", {
						name: leagueName,
						n: String(enrichedTeams.length),
					}),
					false,
				);
			} catch (error) {
				...
				const msg =
					error instanceof Error ? error.message : t("discovery.unknownErr");
				setTeamDiscoveryStatus(
					t("discovery.leagueFailed", { hint, msg }),
					true,
				);
			}
		});
	...
	document.body.addEventListener("click", (event) => {
		...
		const target =
			raw instanceof Element
				? raw
				: raw && raw.nodeType === Node.TEXT_NODE
					? raw.parentElement
					: null;
		if (!target) {
			return;
		}
		const randomDrawBtn = target.closest(".pes-dashboard-random-draw");
		if (randomDrawBtn) {
			const scopeEl = document.getElementById("pes-dashboard-random-scope");
			const scope = scopeEl && scopeEl.value === "season" ? "season" : "all";
			const state = getPesLeagueApplicationState();
			let pool = [];
			if (scope === "season") {
				const seasonId =
					typeof resolveSelectedSeasonId === "function"
						...
						: null;
				if (!seasonId) {
					showToastMessage(t("dash.randomNeedSeason"), "error");
					return;
				}
				const season = findSeasonById(state, seasonId);
				const ids =
					season && Array.isArray(season.playerIds) ? season.playerIds : [];
				pool = ids.map((id) => findPlayerById(state, id)).filter(Boolean);
			} else {
				pool = listAllPlayersSortedByCreatedAt(state);
			}
			if (!pool.length) {
				showToastMessage(t("dash.randomEmptyPool"), "error");
				return;
			}
			const pick = pool[Math.floor(Math.random() * pool.length)];
			const out = document.getElementById("pes-dashboard-random-result");
			if (out) {
				out.textContent = getPlayerDisplayName(pick);
			}
      return;
    }
    const statCompareBtn = target.closest(".pes-stat-compare-run");
    if (statCompareBtn) {
      const a = document.getElementById("pes-stat-compare-a")?.value || "";
      const b = document.getElementById("pes-stat-compare-b")?.value || "";
      if (!a || !b) {
        showToastMessage(t("stats.compareNeedBoth"), "error");
        return;
      }
      if (a === b) {
        showToastMessage(t("stats.compareSame"), "error");
        return;
      }
      const state = getPesLeagueApplicationState();
      const seasonId =
        typeof resolveSelectedSeasonId === "function"
          ...
          : null;
      if (!seasonId) {
        showToastMessage(t("common.pickSeasonHeader"), "error");
        return;
      }
      if (typeof renderPesStatCompareOutput === "function") {
        renderPesStatCompareOutput(state, seasonId, a, b);
      }
      return;
    }
    const editPlayerButton = target.closest(".pes-edit-player");
		if (editPlayerButton) {
			...
			if (playerId) {
				...
				if (typeof renderPlayersView === "function") {
					renderPlayersView(getPesLeagueApplicationState());
				}
				// Two rAFs: wait for layout/paint after profileRoot.innerHTML before measuring scroll.
				window.requestAnimationFrame(() => {
					window.requestAnimationFrame(() => {
						scrollPesPlayerProfileIntoViewIfVisible({ forModalOpen: true });
		...
		if (openPlayerProfileButton) {
			...
			if (playerId) {
				...
				window.requestAnimationFrame(() => {
					window.requestAnimationFrame(() => {
						scrollPesPlayerProfileIntoViewIfVisible();
					});
				});
			}
			return;
		...
		if (deletePlayerButton) {
			...
			if (playerId && window.confirmPesDangerous(t("confirm.deletePlayer"))) {
				const state = getPesLeagueApplicationState();
		...
		if (deleteTeamButton) {
			...
			if (teamId && window.confirmPesDangerous(t("confirm.deleteTeam"))) {
				const state = getPesLeagueApplicationState();
		...
		if (generateFixturesButton) {
			...
			showToastMessage(
				t("toast.fixturesGenerated", {
					count: String(result.createdMatchesCount),
				}),
				"success",
			);
			return;
		...
		if (doubleRoundButton) {
			...
			applyPesLeagueStateAndRefresh(result.state);
			showToastMessage(
				nextFlag ? t("toast.doubleOn") : t("toast.doubleOff"),
				"success",
			);
			return;
		}
		...
		if (cloneSeasonButton) {
			...
			const proposedName = window.prompt(
				t("prompt.newSeasonName"),
				t("prompt.newSeasonDefault"),
			);
		...
		if (deleteSeasonButton) {
			...
			if (
				!window.confirmPesDangerous(
					t("confirm.deleteSeason", { name: seasonLabel }),
				)
			) {
				return;
			}
		...
		if (discoveredTeamButton) {
			const indexValue = discoveredTeamButton.getAttribute(
				"data-discovered-index",
			);
	...
	document.body.addEventListener("click", (event) => {
		...
		if (addCardBtn) {
			...
			list.insertAdjacentHTML(
				"beforeend",
				buildDisciplineCardRowHtml(showCarry, {}),
			);
			return;
		}
...
function bindExportButtons() {
	...
	if (standingsButton) {
		standingsButton.addEventListener("click", () => {
			...
			const season = findSeasonById(state, seasonId);
			const safeName = (season ? season.name : "sezona").replace(
				/[^\w\-]+/g,
				"_",
			);
			downloadTextFile(
...
function bindCloudControls() {
	...
	if (cloudForm) {
		cloudForm.addEventListener("submit", async (event) => {
			...
			const leagueId = String(
				document.getElementById("pes-cloud-league-id").value || "",
			).trim();
			...
			if (!hydrateResult.ok) {
				showToastMessage(
					t("toast.cloudError", { detail: hydrateResult.message }),
					"error",
				);
				refreshCloudSyncStatusUi();
				return;
	...
	if (syncNowButton) {
		syncNowButton.addEventListener("click", async () => {
			...
			if (!result.ok) {
				showToastMessage(
					t("toast.syncError", { detail: result.message }),
					"error",
				);
				return;
			}
